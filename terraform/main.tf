...
resource "aws_subnet" "subnet-nextjs" {
  vpc_id     = aws_vpc.Main-VPC.id
...
resource "aws_route_table_association" "route-to-subnet"{
    subnet_id= aws_subnet.subnet-nextjs.id
    route_table_id= aws_route_table.route_internet_gateway.id
}

resource "aws_security_group" "nextjs-client-security-group" {
    name= "nextjs-client-security-group"
...
resource "aws_instance" "nextjs-client" {
    ami = "ami-0230bd60aa48260c6"
    instance_type = "t2.micro"
    key_name=aws_key_pair.default.key_name
    subnet_id = aws_subnet.subnet-nextjs.id
    vpc_security_group_ids  = [aws_security_group.nextjs-client-security-group.id]
    ...
    docker pull 590654135973.dkr.ecr.us-east-1.amazonaws.com/nextjs-client:latest
    docker run -d --name temp_container 590654135973.dkr.ecr.us-east-1.amazonaws.com/nextjs-client:latest
